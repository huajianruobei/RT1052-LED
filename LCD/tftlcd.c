...
//升级说明
//V1.1 20180430
//修改LCD_Init函数对MCU屏的速度设置，以达到最高刷新率.
//V1.2 20220406
//1，去掉大部分不常用驱动IC的支持
//2，新增对ST7789驱动IC的支持
//3，优化代码结构（精简源码）
//4，修改NT5510 ID读取方式,改为先发送秘钥,然后读取C500和C501,从而获取正确的ID(0X5510)
//5，解决因NT5510 ID读取（发送C501指令）导致SSD1963误触发软件复位进而读取不到ID问题，加延时解决
//////////////////////////////////////////////////////////////////////////////////
...
u32 LCD_ReadPoint(u16 x, u16 y)
{
    ...
    g <<= 8;
    return (((r >> 11) << 11) | ((g >> 10) << 5) | (b >> 11));  //9341/5310/5510/7789需要公式转换一下
}
...
u8 LCD_SEMC_Init(void)
{
    ...
    if (SEMC->BR[6] != 0X00)        //SEMC没有被初始化过(用BR6来表示是否被初始化过,防止时钟重复配置)
    {
        CCM->CCGR3 |= 3 << 4;       //使能SEMC时钟
        CCM->CCGR1 |= 3 << 18;      //使能SEMC_EXSC_CLOCK时钟
        SEMC->MCR |= 1 << 0;        //软复位SEMC

        while (SEMC->MCR & 0X01);   //等待复位结束
        ...
        CCM->CBCDR &= ~(1 << 6);    //选择PERIPH_CLK(600Mhz)为SEMC时钟源
        CCM->CBCDR &= ~(0X07 << 16); //清除SEMC_PODF的设置
        CCM->CBCDR |= 3 << 16;      //SEMC_CLK=输入时钟/(3+1)=600M/4=150Mhz
        CCM->CCGR3 |= 3 << 4;       //使能SEMC时钟
        SEMC->MCR |= (u32)0X1F << 24; //BTO(bus timeout cycle)为:255*2^31;
        SEMC->MCR |= 0 << 16;       //CTO(cmd timeout cycle)为:256*1024;
        SEMC->MCR |= 1 << 2;        //DQS模式为1(dqs loopbacked from dqs pad),必须设置这个,才能到166Mhz
        SEMC->MCR |= 1 << 1;        //禁止SEMC
        tempreg = (0X10 << 16) | (0X40 << 8) | (8 << 4) | 5;
        SEMC->BMCR0 = tempreg;      //队列A设置
        tempreg = (0X40 << 24) | (0X24 << 16) | (0X60 << 8) | (8 << 4) | 5;
        SEMC->BMCR1 = tempreg;      //队列B设置
        SEMC->MCR &= ~(1 << 1);     //使能SEMC
        return 0;
    }
...
void LCD_Init(void)
{
    ...
    //针对MCU屏，必须修改0X80500000开始的SDRAM地址空间属性为不可缓存内存，否则无法正常使用MCU
    //但是对于RGB屏，0X80500000开始的地址，属性不能设置为不可缓存内存，否则RGB屏时钟无法超过40M
    //region 15,起始地址0X8050 0000,SDRAM首地址+5M,相同地址段设置,region大的覆盖小的
    MPU->RBAR = ARM_MPU_RBAR(15, 0x80200000U);
    ...
    IOMUXC_SetPinMux(IOMUXC_GPIO_EMC_19_SEMC_ADDR11, 0); //LCD_WR
    IOMUXC_SetPinMux(IOMUXC_GPIO_EMC_20_SEMC_ADDR12, 0); //LCD_RD
    IOMUXC_SetPinMux(IOMUXC_GPIO_EMC_22_SEMC_BA1, 0);   //LCD_RS
    IOMUXC_SetPinMux(IOMUXC_GPIO_B0_00_SEMC_CSX01, 0);  //LCD_CS

    //LCD背光
    IOMUXC_SetPinMux(IOMUXC_GPIO_SD_B1_02_GPIO3_IO02, 0); //LCD_BL
    ...
    IOMUXC_SetPinConfig(IOMUXC_GPIO_EMC_19_SEMC_ADDR11, 0X110F9);   //LCD_wR
    IOMUXC_SetPinConfig(IOMUXC_GPIO_EMC_20_SEMC_ADDR12, 0X110F9);   //LCD_RD
    IOMUXC_SetPinConfig(IOMUXC_GPIO_EMC_22_SEMC_BA1, 0X110F9);      //LCD_RS

    IOMUXC_SetPinConfig(IOMUXC_GPIO_B0_00_SEMC_CSX01, 0X1D0F9);     //LCD_CS

    //LCD背光
    //配置IO引脚GPIO_SD_B1_02功能
    //低转换速度,驱动能力为R0/6,速度为100Mhz，关闭开路功能，使能pull/keepr
    //选择keeper功能，下拉100K Ohm，关闭Hyst
    IOMUXC_SetPinConfig(IOMUXC_GPIO_SD_B1_02_GPIO3_IO02, 0x10B0);   //LCD_BL

    lcdbl_config.direction = kGPIO_DigitalOutput;   //输出
    lcdbl_config.interruptMode = kGPIO_NoIntmode;   //不使用中断功能
    lcdbl_config.outputLogic = 0;                   //默认低电平,LCD背光关闭
    GPIO_PinInit(GPIO3, 2, &lcdbl_config);          //初始化GPIO3_02
    ...
    dbi_config.columnAddrBitNum = kSEMC_Dbi_Colum_12bit;//DBI列地址12bit
    dbi_config.burstLen = kSEMC_Dbi_BurstLen1;          //突发长度为1
    dbi_config.portSize = kSEMC_PortSize16Bit;          //16位
    dbi_config.tCsxSetup_Ns = 180;                      //CS建立时间,控制CS拉低到WR/RD拉低的延迟,相当于控制地址建立时间
    dbi_config.tCsxHold_Ns = 80;                        //CS保持时间,在WR/RD拉高,并延迟RDH/WRH以后,CS再保持低电平的时间
    dbi_config.tWexLow_Ns = 81;                         //WR低电平时间
    dbi_config.tWexHigh_Ns = 81;                        //WR高电平时间
    dbi_config.tRdxLow_Ns = 81;                         //RD低电平时间
    dbi_config.tRdxHigh_Ns = 101;                       //RD高电平时间
    dbi_config.tCsxInterval_Ns = 60;                    //写数据建立时间,控制CS高电平时间(连续操作时)
    SEMC_ConfigureDBI(SEMC, &dbi_config, semcclock);    //初始化DBI接口
    ...
    if (lcddev.id == 0X9341 || lcddev.id == 0X5310 || lcddev.id == 0X5510 || lcddev.id == 0X1963)   //如果是这几个IC,则设置WR时序为最快
    {
        ...
        if (lcddev.id == 0X9341)
            SEMC->DBICR1 = 1 << 0;  //CS建立时间,控制CS拉低到WR/RD拉低的延迟,2CLK=13.34NS
        else 
            SEMC->DBICR1 = 0 << 0;  //CS建立时间,控制CS拉低到WR/RD拉低的延迟,1CLK=6.67NS
        
        SEMC->DBICR1 |= 0 << 4;     //CS保持时间,在WR/RD拉高,并延迟RDH/WRH以后,CS再保持低电平的时间,1CLK=6.67NS
        SEMC->DBICR1 |= 2 << 8;     //WR低电平时间,3CLK=20.01NS
